...
function draw(x, y) {
  ...
  ctx.moveTo(lastX, lastY); //Déplace le stylo aux coordonnées x et y.
  //go to
  ctx.lineTo(x, y); //Dessine une ligne depuis la position de dessin courante jusqu'à la position spécifiée par x et y (offset position par rapport à l'élément)
  ctx.stroke(); //Dessine la forme en traçant son contour.
  [lastX, lastY] = [x, y]
};
...
//Evénement sur mouvement touch
canvas.addEventListener("touchstart", (e) => { //toucher écran
  const canvasPos = document.getElementById("signature").getBoundingClientRect() //renvoie la taille d'un élément et sa position relative par rapport à la zone d'affichage (viewport)
  let touchX = e.touches[0].clientX - canvasPos.left;
  let touchY = e.touches[0].clientY - canvasPos.top;
  [lastX, lastY] = [touchX, touchY]
  draw(touchX, touchY);
});
canvas.addEventListener("touchmove", (e) => { //déplacement sur écran
  e.preventDefault();
  const canvasPos = document.getElementById("signature").getBoundingClientRect()
  let touchX = e.touches[0].clientX - canvasPos.left;
  let touchY = e.touches[0].clientY - canvasPos.top;
  draw(touchX, touchY);
});
canvas.addEventListener("touchend", () => actionDessiner = false); //fin du tactil
canvas.addEventListener("touchleave", () => actionDessiner = false); //toucher hors canvas
